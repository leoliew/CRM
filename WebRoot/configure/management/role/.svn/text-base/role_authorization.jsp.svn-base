...
function click_save(){
  var form=getForm();
    for (var i=0; i<form.moduleId.options.length; i++) {
      if (form.moduleId.options[i].text.indexOf("√")!=-1) {
      //将选中的值放到隐藏表单中
        form.rolemodule.value = form.rolemodule.value+form.moduleId.options[i].value+";";
      }
    }
...
function checkselect()
{
  var form=document.all(formName);
  var i;
  var iselect = form.moduleId.options.selectedIndex;
  //alert("iselect -->" + iselect);
  var seltext = form.moduleId.options[iselect].text;
  //alert("seltext -->" + seltext);
  var selvalue = form.moduleId.options[iselect].value;
  //alert("selvalue -->" + selvalue);
  charindex = seltext.indexOf("√");
  //alert("charindex -->" + charindex);
  if (charindex != -1)
  {
  ...
  form.moduleId.options[iselect].text = seltext;
}

function chooseAll(){
	var form=getForm();
	var isCheck = form.checkall.checked;
	if(isCheck == true){
		for (var i=0; i<form.moduleId.options.length; i++) {
			if (form.moduleId.options[i].text.indexOf("√") == -1) {
        		form.moduleId.options[i].text = form.moduleId.options[i].text + " √";
      		}
		}
	}
}
...
<body vlink=#000000 alink=#000000 link=#000000 bgColor=#ffffff>
  <!-- 栏目添加部分开始 -->
  <form action="roleManageAction.do" method="post" name="roleManagerActionForm">
  	<input type="hidden" name="rolemodule"> 
			...
			<table border="0" cellspacing="0" cellpadding="0" background="/images/di03.gif" width="100%">
			  <tr>
				<td width="100%">
				  <table border="0" cellspacing="1" cellpadding="2" bgcolor="#CADADF" width="100%">
					<tr bgcolor="#F4F7FA">
					<td><input type="checkbox" name="checkall"  value="all" onClick="chooseAll()">全选</td>
					</tr>
					<tr bgcolor="#F4F7FA">
					  <td align="left">
					  <select name="moduleId" size='15' style="height:200px;width:100%" onclick="checkselect()">
						  <c:forEach items="${requestScope.allModuleList}" var="module">
